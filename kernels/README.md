...
| `bf16_activation_sweep/` | ReLU/SiLU/GELU (the bf16 activation engine) | **Loses at every op and size, end to end**, best case 0.77×. Correct and accurate (SiLU 5.5%, GELU 4.0% max rel error); the LUT ops are core-limited at 8–11 GB/s where ReLU reaches 52. Standalone dispatch only — fused untested |
| `int8_matmul_sweep/` | Nothing — the int8 GEMM sweep, with the CPU int8 GEMM baseline; its driver also runs `--dtype i16`/`bf16` | **Loses at the default tile; wins 1.10×–1.83× at M ≥ 512, N ≥ 2048 with `n=64`**, a tile bf16 can't fit. The int16 pass found **no dtype penalty to measure** (0.68–1.04× int8, indistinguishable from bf16) and that **int16×int16 is native on AIE2** after all — the missing `device.yaml` row is a cost-model gap, settled by disassembly |
