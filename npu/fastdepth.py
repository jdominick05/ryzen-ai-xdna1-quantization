"""FastDepth monocular depth estimation preprocess/postprocess, in one place.
...
Preprocesses images using standard [0, 1] scaling (RGB / 255.0) and provides
...
def preprocess(img_bgr, target_size=INPUT_SIZE):
    """BGR uint8 HWC -> NCHW float32 [0, 1], plus original (orig_h, orig_w).

    Resizes via cv2.INTER_LINEAR, converts BGR to RGB, scales to [0, 1],
    and returns a contiguous (1, 3, target_size, target_size) array.
    """
...
def postprocess_depth(raw_depth, orig_shape=None, normalize=True):
    """Network output -> relative depth map.
