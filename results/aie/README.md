...
[`notes_persistent_weight_optimization.md`](notes_persistent_weight_optimization.md) — architectural decoupling of the monolithic AIE2 transaction stream into a one-time parameter initialization binary (`build/layer_conv0_init.bin`, 62,528 B) and a lightweight per-frame execution binary (`build/layer_conv0_exec.bin`, 10,496 B, plus an ultra-compact `< 2.5 KB` buffer `build/layer_conv0_exec_minimal.bin`, 1,920 B). Eliminates 39,024 B of stationary weights, biases, and shift parameters, 3,328 B of static switchbox programming, and 4,608 B of redundant BD zeroing writes per frame (46,960 B eliminated per frame). Benchmarked on physical AMD Phoenix XDNA1 silicon (Ryzen 7 8700G, NPU `[003d:00:01.1]`, tile clock 1.80 GHz) over 500 pipelined iterations: scales pipelined throughput from 5,054.7 FPS (197.84 µs) to **11,578.1 FPS** (**86.37 µs**, **2.29× speedup** over monolithic pipelining) and reduces synchronous latency from 308.98 µs to **164.04 µs** (**1.88× speedup**). Retains **100.00% bit-exact numerical parity ($2,048/2,048$ bytes, $\text{MAE} = 0.0000$, $\text{RMSE} = 0.0000$, $\text{MaxAE} = 0$)** against the exact INT8 QDQ reference and 100.00% deterministic ring parity across both Ping and Pong buffer sets. Execution trace logged in [`hardware_16core_persistent_verification.log`](hardware_16core_persistent_verification.log).
