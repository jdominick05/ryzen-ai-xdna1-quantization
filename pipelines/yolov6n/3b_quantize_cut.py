"""
YOLOv6n step 3b: quantize the head-cut model (from 1b_cut_head.py) to XINT8.

There is no float tail left in the graph, so there is nothing to exclude --
every node is a candidate for the NPU.

    conda activate resnet_env
    python pipelines/yolov6n/3b_quantize_cut.py --calib-dir data/coco_calib
    python pipelines/yolov6n/3b_quantize_cut.py --calib-dir data/coco_calib --adaround

Writes models/yolov6n_cut_xint8.onnx (or _xint8_adaround.onnx).
"""
import argparse
import copy
import glob
...
from npu.yolov6 import HEAD_OUTS, input_size, letterbox

IN_MODEL = MODELS / "yolov6n_cut.onnx"

# Same AdaRound params yolov8n's cut quantizer uses (AMD's own defaults).
ADAROUND = {
...
class CocoCalibReader(CalibrationDataReader):
    """Uses npu.yolov6.letterbox (== npu.yolo.letterbox), the same transform
    inference uses. Do not inline a second copy."""
