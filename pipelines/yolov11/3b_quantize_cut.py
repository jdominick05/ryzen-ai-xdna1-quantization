"""
YOLOv11 step 3b: quantize the head-cut model (from 1b_cut_head.py) to XINT8.

Difference from stock YOLOv8/v6: YOLOv11 contains the C2PSA spatial self-attention
block (4D MatMul + Softmax + Transpose) in the backbone. Quark XINT8 will quantize
the Conv layers around it and insert QDQ nodes.

    conda activate resnet_env
    python pipelines/yolov11/3b_quantize_cut.py --calib-dir data/coco_calib --limit 64
    python pipelines/yolov11/3b_quantize_cut.py --calib-dir data/coco_calib --adaround

Writes models/yolo11n_cut_xint8.onnx (or _xint8_adaround.onnx).
"""
import argparse
import copy
import glob
...
from npu.yolov11 import HEAD_OUTS, input_size, letterbox

IN_MODEL = MODELS / "yolo11n_cut.onnx"
...
class CocoCalibReader(CalibrationDataReader):
    """Uses npu.yolov11.letterbox, the same transform inference uses. Do not
