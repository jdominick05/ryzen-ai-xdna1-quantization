...
execution provider. Three pipelines — ResNet50 classification, YOLOv8 n/s/m/l/x detection, YOLOv8n-pose keypoints — each `export → quantize → run → measure`.
...
| `quant/` (Ignition) | Folded ResNet, with or without CLE, plus AdaRound | **Alpha 0.1.0a1** — independent calibration/emission without Quark or torch; `--cle` reproduces the repo's plain-XINT8 ResNet50 to the integer, and `adaround` reproduces Quark's `XINT8_ADAROUND` byte for byte on the same machine. [Quickstart](quant/README.md), [todo list](quant/TODO.md), [validation](docs/BENCHMARKS.md#ignition-alpha-release-validation), [CLE parity](docs/BENCHMARKS.md#ignition-cle-parity-and-the-default-xint8-preset), [AdaRound parity](docs/BENCHMARKS.md#ignition-adaround-parity), [acceptance findings](docs/BENCHMARKS.md#ignition-controlled-resnet-qdq-acceptance) |
