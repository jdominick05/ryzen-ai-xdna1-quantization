"""
MiDaS step 3: Quantize MiDaS v2.1 Small to XINT8 (or XINT8 + AdaRound).

Uses npu.midas.preprocess, ensuring calibration and inference are byte-identical.
...
to prevent SMT thrashing during calibration / FastFinetune.
    ...
    python pipelines/midas/3_quantize.py --calib-dir data/midas_calib
    python pipelines/midas/3_quantize.py --calib-dir data/midas_calib --adaround
"""
import argparse
import copy
import glob
...
from npu.midas import INPUT_SIZE, input_size, preprocess
from npu.paths import MODELS

DEFAULT_IN = MODELS / "midas_small_nearest_cut.onnx"
...
class MidasCalibReader(CalibrationDataReader):
    """Feeds calibration images using npu.midas.preprocess."""
...
def main():
    ...
    ap.add_argument("--calib-dir", default=str(Path("data") / "midas_calib"))
    ...
    dr = MidasCalibReader(args.calib_dir, args.limit, target_size=target_size, input_name=input_name)
