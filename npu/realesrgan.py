"""Real-ESRGAN super-resolution preprocess/postprocess, tiling, and fidelity metrics.

Quark-free by the `npu/` invariant, and cv2/numpy only so it imports in `resnet_env17`
(which has no torch and no Pillow) as well as in `resnet_env`.

Preprocesses images into normalized [0.0, 1.0] RGB NCHW float32 tensors and provides
tile splitting, seamless blending reconstruction, and Y-channel PSNR (dB) and SSIM
evaluation utilities for 4x SISR restoration pipelines.
"""
import math
import cv2
import numpy as np
...
def preprocess(img_bgr, target_size=DEFAULT_INPUT_SIZE):
    """BGR uint8 HWC -> NCHW float32 in [0.0, 1.0], plus original (orig_h, orig_w).

    Resizes to (target_size, target_size) via cv2.INTER_LINEAR if not matching,
    converts BGR to RGB, scales by 1/255.0, and returns a contiguous (1, 3, H, W) array.
    """
...
def postprocess(pred_chw):
    """Network output (NCHW or CHW float32 in [0.0, 1.0]) -> BGR uint8 image.

    Clips to [0.0, 1.0], denormalizes to [0, 255], and converts RGB to BGR.
    """
    ...
    pred_u8 = np.clip(pred_rgb * 255.0 + 0.5, 0.0, 255.0).astype(np.uint8)
...
def split_into_tiles(img_bgr, patch_size=(DEFAULT_INPUT_SIZE, DEFAULT_INPUT_SIZE), overlap=8):
    ...
    if core_h <= 0 or core_w <= 0:
        raise ValueError(f"Overlap {overlap} is too large for patch size {patch_size}")
