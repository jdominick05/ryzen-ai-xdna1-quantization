...
| `aie/` | `notes_aie2_device_dtypes.log` -- not a measurement, a verbatim excerpt of AMD's own `OGOAT/Collaterals/device.yaml` (shipped in the 1.7.1 install) giving Phoenix's `AIE2` tile spec per data type, plus the `aie4_models` dead-end (internal AMD kernel-source tree, wrong chip generation); `aiecompiler_*.log` -- a real `aiecompiler` bring-up against the trivial graph in `tools/aie_probe/`: the compiler runs and a graph compiles up to needing a `--part`/`--platform` device-model string, and none exists anywhere in this SDK (three candidates tried, all rejected) | RESEARCH.md: "Custom C++ XRT / hand-written AIE kernels" |
