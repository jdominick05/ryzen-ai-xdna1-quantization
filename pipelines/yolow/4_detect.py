"""
YOLO-World v2 step 4: run detection on an image, video or webcam, on CPU, DML or NPU.

Handles both graph shapes:
  * full graph  (1 output, `output0`)  - the ONNX decode tail runs in the model
  * head-cut    (6 outputs, HEAD_OUTS) - visual features + box features; contrastive
    dot-product and DFL tail run in numpy via npu.yolow.decode_yolow

    conda activate resnet_env17
    $env:RYZEN_AI_INSTALLATION_PATH = 'C:\\Program Files\\RyzenAI\\1.7.1'

    python pipelines/yolow/4_detect.py --model models/yolov8s-worldv2.onnx --ep cpu --source assets/test_image.jpg
    python pipelines/yolow/4_detect.py --model models/yolov8s-worldv2_cut_xint8.onnx --ep npu --source assets/test_image.jpg --fresh --log 1
    python pipelines/yolow/4_detect.py --model models/yolov8s-worldv2_cut_xint8.onnx --ep dml --source assets/test_image.jpg
"""
import argparse
import os
import sys
...
from npu.paths import RESULTS, yolow_cache_key
...
def main():
    ...
    ap.add_argument("--source", default="assets/test_image.jpg", help="image path, video path, or camera index")
    ap.add_argument("--txt-feats", default=None, help="path to custom normalized text embeddings (.npy)")
    ...
    # Load text embeddings (defaults to COCO 80 categories)
    ...
    if n_out == len(yw.HEAD_OUTS):
        ...
        print(f"model: head-cut ({n_out} outputs), contrastive + DFL decode runs in numpy")

        def run_raw(x):
            return sess.run(None, {inp: x})

        def decode_raw(r):
            reordered = [r[i] for i in order]
            return yw.decode_yolow(reordered, txt_feats, imgsz=imgsz,
    ...
    if is_img:
        ...
        x, pad, scale = yw.letterbox(orig, imgsz)
        # Warmup
        for _ in range(3):
        ...
        dets = yw.postprocess(preds, pad, scale,
        ...
        for x0, y0, w, h, s, c in sorted(dets, key=lambda d: -d[4])[:10]:
            cls_name = yw.COCO_CLASSES[c] if c < len(yw.COCO_CLASSES) else str(c)
    ...
    while True:
        ...
        if not ret:
            break
        x, pad, scale = yw.letterbox(frame, imgsz)
        t0 = time.perf_counter()
        raw = run_raw(x)
        preds = decode_raw(raw)
        dets = yw.postprocess(preds, pad, scale,
