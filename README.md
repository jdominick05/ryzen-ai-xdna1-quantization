...
> **The headline finding is that the edge is energy, not speed.** On ResNet50 the NPU runs **19.6 inferences per joule against the iGPU's 3.3 and the CPU's 1.7 — 11.3× the CPU, 5.9× the iGPU** — while being only 2.8× and 1.34× faster. On BiSeNetV2, the one model where the NPU's *latency* lead over the iGPU is narrowest (and flips direction session to session), the *energy* lead never wavers: 5.0× the iGPU (retracted-and-corrected; see BENCHMARKS). A CPU-on-the-same-INT8-artifact control rules out "int8 is just cheaper" on both. This reframes every latency-only verdict below: [joules per frame](docs/BENCHMARKS.md#joules-per-frame-the-npu-does-113-the-inferences-per-joule-of-the-cpu-59-the-igpu), [a second, adversarial model](docs/BENCHMARKS.md#a-second-model-chosen-to-be-adversarial-to-the-finding-the-energy-edge-survives-where-the-speed-edge-nearly-vanishes), [the first watt](docs/BENCHMARKS.md#the-first-watt-the-npu-is-133-faster-and-445-cheaper-on-the-same-gemm).
...
- **int16 costs nothing against int8**: 0.68×–1.04× int8 and within 3% of bf16, on a quarter
  the MACs per `vmac`. All three run under half their MAC ceiling — the tile is dtype-blind.
