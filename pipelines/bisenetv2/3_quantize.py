"""Quantize BiSeNetV2 to Quark XINT8 for AMD XDNA1 NPU.

Uses Quark 0.11rc1 with PowerOfTwoMethod.MinMSE symmetric power-of-two
quantization matching AMD VitisAI EP requirements.
"""
import argparse
import copy
import glob
...
from npu.bisenetv2 import INPUT_SIZE, input_size, preprocess
from npu.paths import MODELS

DEFAULT_IN = MODELS / "bisenetv2_fp32.onnx"
...
class BiSeNetCalibReader(CalibrationDataReader):
    """Feeds calibration images using npu.bisenetv2.preprocess."""
    ...
    def get_next(self):
        if self.i >= len(self.files):
            return None
        img_bgr = cv2.imread(self.files[self.i])
        self.i += 1
        if img_bgr is None:
            return self.get_next()
        tensor, _ = preprocess(img_bgr, target_size=self.target_size)
...
def main():
    ...
    ap.add_argument("--calib-dir", default=str(REPO_ROOT / "data" / "bisenetv2_calib"))
    ...
    dr = BiSeNetCalibReader(args.calib_dir, args.limit, target_size=target_size, input_name=input_name)
