...
def finetune(float_graph: Graph, quant_graph: Graph, source: ImageFolderSource,
    ...
    for layer in layers:
        ...
        if batch_size < 1 or batch_size > q_input.shape[0]:
            ...
            batch_size = 1
        # TWO INDEPENDENT FINDINGS, COMBINED rather than chosen between. They are on
        # different axes and neither costs the other anything:
        #   research/windows-lowlevel ("AdaRound peak memory: 16.7% off ResNet50 with the
        #     output bit-identical") DROPPED the inputs_f list the vendor source builds
        #     here. It is assigned and never read -- DropRatio 1 feeds the module
        #     quantized inputs only -- so holding it pinned a third activation set alive
        #     for the whole layer. Re-verified unread on BOTH sides of this merge before
        #     dropping it here.
        #   worktree-ignition-dev ("AdaRound takes an OptimDevice") STAGED the lists on
        #     the device once, before the loop, so the per-iteration torch.cat and the
        #     1000 forward/backward passes never touch host memory.
        # So: stage on the device, and do not build inputs_f at all. Keeping either side
        # alone would have discarded the other's measured win.
