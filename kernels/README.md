...
| `conv2dk1`/`conv2dk1_skip` width fix | Not a directory — a second, separate width-32 bug in the same `bottleneck.py` design's other two kernels, found while re-testing `conv2dk3`'s fix | **RESOLVED.** `conv2dk1_i8_vector`/`conv2dk1_ui8_vector` hardcoded `iw_32_rem = 0` (a real value, never computed from `input_width` — dead code, not corrupted code); `conv2dk1_skip_i8_vector`'s remainder path was never implemented at all (a commented-out stub). Both guarded only by an `assert` compiled out in release, so any width not a multiple of 32 silently dropped its remainder columns. Rewrote all three to walk `input_width` in blocks of N≤4 chunks addressed directly from base pointers (no incremental pointer state), same accumulator-count fix as `conv2dk3`. `conv2dk1_skip_ui8_vector` (uint8-skip twin, unused by `bottleneck.py`) left unfixed and flagged. Verified end-to-end through the real `bottleneck.py` design via `kernels/bottleneck_sweep/sweep.py`'s torch-golden gate: 32×32/36/40/44 all pass (previously failed at 36/40/44 even with `conv2dk3` alone fixed). Real ceiling for this channel config is `tensor_w`=44 (45 fails the VMAC 4-pixel granularity, not this fix; 46/48 exceed Tile(0,4)'s 64 KB) *before* the buffering change below. See `results/aie/bottleneck_widthfix_npu.log`, `docs/DECISIONS.md`. |
| `bottleneck.py` 56×56 (2026-09-07) | Not a directory — the buffering change that reached ResNet50's actual conv2_x shape, the item left explicitly open in `bottleneck_sweep/`'s own log ("untestable without rewriting the design's buffering") | **REACHED, and the verdict got worse, not better.** Single-buffered the final output `ObjectFifo` (`depth=1` instead of the default 2, in `bottleneck.py` itself) — the one buffer of Tile(0,4)'s five safe to shrink (`skip_buf`'s depth is load-bearing for the skip connection's timing; this one isn't). Frees 3584 B of headroom at w=56, just enough. Both 32×56 and 56×56 verified against the torch golden. At 56×56: NPU hw 4.2435 ms vs CPU 0.3327 ms — **CPU wins 12.75×** (marginal rate 111.1 vs 1678.8 GOPS, **15.1×**), worse than the 5.7–11.4× found at every compile-limited 32-wide shape. See `results/aie/bottleneck_w56_npu.log`, `docs/DECISIONS.md`. |
