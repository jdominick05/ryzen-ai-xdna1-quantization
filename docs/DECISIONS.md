...
- **Theorem 2 (Multi-Branch Inter-Scale Alignment)**: In multi-branch elementwise operations (such as Bilateral Guided Aggregation in BiSeNetV2), divergent scale grids truncate dynamic range in hardware. **Not supported as stated (2026-09-09).** BiSeNetV2's divergence was localised to the single node `/bga/Mul`, whose CPU/NPU correlation falls from 0.986 at both inputs to 0.687 at its output, while its sibling gate `/bga/Mul_1` goes 0.990 to 0.998 -- and the sibling has the *larger* position gap, 6 against 4, with both at feasible sigma. The failing Mul is instead 16x larger spatially. [Localisation, the refuted gain hypothesis and what remains undecided](BENCHMARKS.md#bisenetv2s-cpunpu-gap-localised-to-one-mul-2026-09-09-desktop-2).
