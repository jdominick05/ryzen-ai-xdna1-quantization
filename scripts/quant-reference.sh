#!/usr/bin/env bash
# Build a fresh no-CLE ResNet50 Quark reference, solely for the owned-emitter gate.
#
#   ./scripts/quant-reference.sh --out models/resnet50_quark_nocle_c64.onnx --log results/quant/quant_resnet50_quark_nocle_c64.log [--limit 64]
#
...
quark_guard
run_logged "$LOG" python pipelines/resnet50/3d_quantize_compare.py --out "$OUT" --limit "$LIMIT"
