...
that rather than assumed it: `Estimated Power : N/A` in every `xrt-smi examine` capture
taken here (MEASURED `results/aie/xrt_smi_platform_pmode.log`,
`results/aie/clock_probe_npu.log`), and the electrical query itself failing at the driver
escape (MEASURED `results/aie/xrt_api_live_clock_and_pdh_npu.log`; `docs/SETUP.md` lists it
as the one thing the monitor cannot show). So the measurable quantity is a package-power
...
Decides: whether the NPU's edge is work per watt where it is not work per second. Each
measured CPU loss becomes a different question if the package draw differs — MobileNetV2
at 1.72 ms CPU against 2.68 ms NPU, `resnetv2_50x3_bit` at 470.79 against 588.58 ms, and
yolov8n at 9.9–10.5 ms on the iGPU against 6.8–6.9 ms on the NPU. It also decides whether
...
its own README headline disagrees with its documented method by better than 2× on the same
claim, so none of its numbers travels into this file. What corroborates the paragraph
...
- Per-NPU voltage and power: `Estimated Power` reads N/A and the electrical query fails at
  the driver escape (`results/aie/xrt_api_live_clock_and_pdh_npu.log`), with the Linux
  side reporting the firmware telemetry buffer unpopulated too (amd/xdna-driver#1447)
  — S4 measures a package delta instead.
