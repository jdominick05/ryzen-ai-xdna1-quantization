...
| `bottleneck_sweep/` | Not an operator — the **spatial sweep** the conv2x log asked for: does NPU GOPS scale with problem size, or is conv simply slower here? `sweep.py` runs one standalone `ml/bottleneck` on the NPU; `cpu_sweep.py` runs the same arithmetic through ORT QDQ int8; same shapes, one sitting | `tensor_h` is the free axis (every L1 buffer is `tensor_w`×channels). NPU hw throughput rises **116.7 → 143.7 GOPS** over 16× more work, marginal **146.1** vs the CPU's **819.0** — the CPU wins **7.6×** at 32×32 and **5.7×** at 512×32, its own worst point. Utilization was worth 23% against a 570% gap, so **the op class is closed**. `tensor_w`=32 is a hard `aiecc` ceiling (Tile(0,4) needs 5×`w`×256 B + stack > 64 KB), so ResNet50's real 56×56 conv2_x **cannot compile** on this design. `conv2dk1.cc`/`conv2dk3.cc` were read (2026-09-07): both vectorize correctly with `aie::mmul`, ruling out an attention-kernel-style bug — but `conv2dk3_i8_vector` also hardcodes `const int iw = 32` and ignores its own `input_width` arg in ~20 pointer-stride computations, verified against its own `"results are wrong"` TODO and the scalar variant (which does use `input_width` correctly). Never triggered today only because the L1 ceiling above already rejects those shapes at compile time first (`results/aie/bottleneck_spatial_sweep_npu.log`, `docs/DECISIONS.md`). |
| `conv2dk3_widthfix/` | Not an operator — an isolated single-worker test chasing a real (partial) fix for the `conv2dk3` `iw=32` hardcode above, patching a local (never-pushed) mlir-aie checkout | `design.py` runs one `conv2dk3_ui8_vector` core alone (64→64, no skip-add, `Tile(0,3)`); `test_width.py` checks it against a torch golden built with small weights/inputs specifically so a broken kernel can't pass by output saturation. `config.cxx_header_path()` resolves to `~/mlir-aie`'s installed **wheel**, not its git clone — the first patch attempt silently compiled nothing (patched the wrong copy). Patched correctly, with width as a true `-DINPUT_WIDTH=N` compile-time constant: **32×32 PASS bit-exact**, and at 32×36 (remainder-only path) **7 of 8 output-channel blocks are bit-exact correct** — only channel-block 0's leftmost 4 pixels are wrong. 32×40 (the "aligned" 32-pixel-chunk block, never exercised before this) is still broken throughout. Two scoped bugs remain open, not reverted (net improvement over stock). See `docs/DECISIONS.md` for the full readout and next steps. |
