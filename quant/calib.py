...
from .pow2 import TensorQ, dequantize, pos2scale, qrange, quantize, scale2pos, sqerr
from .qdq import prunable_tensors, quantizable_tensors
from .sources import ImageFolderSource, StreamingHistogramAccumulator
...
def choose_pow2_minmse_hist(vmin: float, vmax: float, counts: np.ndarray, dtype: str,
                            num_bins: int = 2048) -> tuple[int, dict]:
    """Evaluate MinMSE power-of-two candidate scales over uniform histogram bins."""
    low, high = qrange(dtype)
    zp = 128 if dtype == "uint8" else 0
    vmin = float(vmin)
    vmax = float(vmax)
    ...
    if total_count <= 0:
        raise ValueError("MinMSE histogram requires positive sample count")
    ...
    if vmax == vmin:
        errors = [0.0 for _ in candidates]
        chosen = base
        return chosen, {"count": total_count, "vmin": vmin, "vmax": vmax,
                        "minmax_pos": base, "candidate_positions": candidates,
                        "candidate_sqerr": errors, "minmse_pos": chosen}

    delta = (vmax - vmin) / num_bins
    centers = (vmin + (np.arange(num_bins, dtype=np.float32) + 0.5) * delta).astype(np.float32)

    errors = []
    for pos in candidates:
        restored = dequantize(quantize(centers, pos, zp, dtype), pos, zp)
        bin_err = (restored - centers) ** 2
        total_err = float(np.sum(counts.astype(np.float64) * bin_err, dtype=np.float64))
        errors.append(total_err)

    if not all(np.isfinite(errors)):
        raise ValueError("MinMSE float64 histogram error accumulation overflowed")
    chosen = candidates[min(range(len(errors)), key=errors.__getitem__)]
    return chosen, {"count": total_count, "vmin": vmin, "vmax": vmax,
                    "minmax_pos": base, "candidate_positions": candidates,
                    "candidate_sqerr": errors, "minmse_pos": chosen}
...
def collect_and_choose(g: Graph, source: ImageFolderSource, scratch: Path | None = None,
                       *, method: str = "hist", num_bins: int = 2048) -> tuple[dict[str, TensorQ], dict]:
    """Run the float graph to collect activations and choose all quantization positions.

    Supported methods:
      - 'hist': In-memory streaming uniform histogram calibration. Pass 1 collects
        activation dynamic ranges, Pass 2 accumulates into uniform histograms. Reduces I/O
        from O(samples * elements * 2 B) to O(bins * tensors), writing 0 bytes to scratch/%TEMP%.
      - 'exact': Disk-cached float16 sample spooling to temporary files under scratch.
    ...
    if method not in ("hist", "exact"):
        raise ValueError(f"Unknown calibration method: {method!r}, must be 'hist' or 'exact'")
    ...
    if method == "hist":
        print(f"CALIBRATION (streaming histogram) images={len(source)} tensors={len(acts)} "
              f"skipped_prunable={len(pruned)} bins={num_bins} disk_io=0B", flush=True)
        acc = StreamingHistogramAccumulator(num_bins=num_bins)
        # Pass 1: Collect activation dynamic range [vmin, vmax] per tensor
        count = 0
        for count, image in enumerate(source, 1):
            ...
            for name, output in zip(acts, outputs, strict=True):
                ...
                acc.update_range(name, output)
            if count % 16 == 0 or count == len(source):
                print(f"RANGE_COLLECT {count}/{len(source)}", flush=True)
        if count != len(source):
            raise ValueError("Calibration source length changed during range collection")

        # Pass 2: Stream forward activations directly into 2048-bin uniform histograms
        acc.init_histograms()
        count = 0
        for count, image in enumerate(source, 1):
            outputs = session.run(acts, {source.input_name: image})
            for name, output in zip(acts, outputs, strict=True):
                acc.accumulate(name, output)
            if count % 16 == 0 or count == len(source):
                print(f"HIST_COLLECT {count}/{len(source)}", flush=True)
        if count != len(source):
            raise ValueError("Calibration source length changed during histogram accumulation")
        del session, outputs

        report = {"store": f"streaming_histogram_{num_bins}", "mode": "All", "method": "hist",
                  "images": len(source), "listing": [p.as_posix() for p in source.listing()],
                  "num_bins": num_bins, "spooled_tensors": len(acts),
                  "skipped_prunable_tensors": len(pruned), "disk_spool_bytes": 0,
                  "ort_version": ort.__version__, "graph_optimization": "ORT_DISABLE_ALL",
                  "tensors": {}, "sharing": sharing}

        for i, name in enumerate(acts, 1):
            vmin, vmax, counts = acc.get_histogram(name)
            pos, stats = choose_pow2_minmse_hist(vmin, vmax, counts, "uint8", num_bins=num_bins)
        ...
        report = {"store": "exact_float16", "mode": "All", "method": "exact",
                  "images": len(source), "listing": [p.as_posix() for p in source.listing()],
