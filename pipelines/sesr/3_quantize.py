"""
SESR step 3: Quantize SESR-M7 to XINT8 (or XINT8 + AdaRound).

Uses npu.sesr.preprocess, ensuring calibration and inference are byte-identical.
...
to prevent SMT thrashing during calibration / FastFinetune.
    ...
    python pipelines/sesr/3_quantize.py --calib-dir data/sesr_calib
    python pipelines/sesr/3_quantize.py --calib-dir data/sesr_calib --adaround
"""
import argparse
import copy
import glob
...
from npu.sesr import INPUT_SIZE, input_size, preprocess

DEFAULT_IN = MODELS / "sesr_m7_fp32.onnx"
...
class SesrCalibReader(CalibrationDataReader):
    """Feeds calibration images using npu.sesr.preprocess."""

    def __init__(self, folder, limit, target_size=INPUT_SIZE, input_name="input"):
...
def main():
    ...
    ap.add_argument("--calib-dir", default=str(Path("data") / "sesr_calib"))
    ...
    dr = SesrCalibReader(args.calib_dir, args.limit, target_size=target_size, input_name=input_name)
